...
void runLinearTicks(long targetTicks, bool backward);
void runTurnTicks(long targetTicks, bool rightTurn);
...
void runLinearTicks(long targetTicks, bool backward) {
  ...
  while (true) {
    noInterrupts();
    unsigned long l = encoderLeftCount;
    unsigned long r = encoderRightCount;
    interrupts();
    ...
    if (Serial.available()) {
      ...
      switch (cmd) {
        case 'w': { // forward inches
          ...
          runLinearTicks(ticks, /*backward=*/false);
          break;
        }
        case 'b': { // backward inches
          ...
          Serial.print(F(" in -> ")); Serial.print(ticks); Serial.println(F(" ticks"));
          runLinearTicks(ticks, /*backward=*/true);
          break;
        }
        case 'r': { // right turn degrees
          ...
          runTurnTicks(ticks, /*rightTurn=*/true);
          break;
        }
        case 'l': { // left turn degrees
          ...
          Serial.print(F(" deg -> ")); Serial.print(ticks); Serial.println(F(" ticks"));
          runTurnTicks(ticks, /*rightTurn=*/false);
          break;
        }
        case 'x': { // stop now
