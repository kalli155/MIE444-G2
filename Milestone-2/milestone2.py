...
def sensor_readings():
    ...
    global frontL
    print(u1)
    frontL = float(u1.decode('utf-8').strip()) / 2.54
    time.sleep(LOOP_PAUSE_TIME)
    
    SER.write(b'u2\n')
    u2 = SER.readline()
    global left
    left = float(u2.decode('utf-8').strip()) / 2.54
    time.sleep(LOOP_PAUSE_TIME)

    SER.write(b'u3\n')
    u3 = SER.readline()
    global frontR
    frontR = float(u3.decode('utf-8').strip()) / 2.54
    time.sleep(LOOP_PAUSE_TIME)

    SER.write(b'u4\n')
    u4 = SER.readline()
    global right
    right = float(u4.decode('utf-8').strip()) / 2.54
    time.sleep(LOOP_PAUSE_TIME)
    ...
    print("front L: ", frontL, "front R:", frontR)
...
def pause():
    ...
    while running:
        ...
        response_str = str(response.decode('utf-8').strip())
        if response_str == '[RES] Good':
            running = False
    time.sleep(LOOP_PAUSE_TIME)
...
def wall_dis():
    ...
    if 1 <= measurement <= 3:
        wall = 0
    elif 13 <= measurement <= 15:
        wall = 1
    elif 25 <= measurement <= 27:
        wall = 2
    elif 37 <= measurement <= 39:
        wall = 3
    elif 49 <= measurement <= 51:
        wall = 4
    elif 61 <= measurement <= 63:
        wall = 5
    else:
        face_north("l")
        pause()
...
if __name__ == '__main__':
    Thread(target = sensor_readings).start()
